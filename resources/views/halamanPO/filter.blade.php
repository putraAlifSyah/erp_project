...
<div class="container">
    ...
    <div class="pull-right">
        <form action="/po/filter" style="display: flex" method="get"> 
            ...
            <select name="filter" class="form-control mb-1 mr-2" id="filter" style="display: inline">
                <option value="">{{ $datafilter->nama_vendor }}</option>
                    @foreach ($dataVendor as $item)
                        @if ($item->nama_vendor != $datafilter->nama_vendor)
    ...
    <form action="/po/proses" style="display: flex" method="get"> 
        <p style="margin-right: 50px">Total: </p><input type="text" class="" value="Rp. {{ $totalHargaSemua }}">
        <input type="hidden" name="namavendor" id="" value="{{ $nama_vendor->nama_vendor }}" >
        <button style="margin-left: 5px" class="btn-sm btn-warning rounded mt-0.5" type="submit" id="button-addon2" style="height: 35px">{{ $namaButton }}</button>
        </form>
    </div>
    </div>
    </div>
